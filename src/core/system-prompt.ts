...
export const getSystemPromptTemplate = (): string => DEFAULT_SYSTEM_PROMPT_TEMPLATE;

export const buildSystemPrompt = (
  promisePhrase: string,
  template?: string
): string => {
  const source = template ?? getSystemPromptTemplate();
  return source.split("{{PROMISE}}").join(promisePhrase);
};
